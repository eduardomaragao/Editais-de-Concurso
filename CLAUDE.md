...
Quase pronto — guia completo em `docs/PUBLICAR.md`. **Feito:** GitHub (`github.com/eduardomaragao/Editais-de-Concurso`), backend em produção no Render (`https://editais-backend-o6vu.onrender.com`, painel trancado, `/api` público confirmado), painel admin com Basic auth, ícone do app, chave de assinatura real gerada por Eduardo (`upload-keystore.jks` + `key.properties`, gitignored), `app-release.aab` já buildado e assinado apontando para produção, política de privacidade publicada como site estático (`site-legal/index.html`, serviço `editais-legal` no `render.yaml` — confirmar no painel do Render se subiu sozinho ou se pede aprovação do Blueprint), ficha da loja com e-mail confirmado (`prof.eduardoaragao@gmail.com`) e nome do operador ("Eduardo Aragão" — **confirmar se está correto**, foi inferido do domínio). **Bug real encontrado e corrigido durante sessão de capturas de tela:** dropdown de matéria no relógio de estudo estourava a tela em celulares estreitos (faltava `isExpanded: true`) — só apareceu em viewport mobile real, não no `flutter analyze`. **Nota: painel admin (código de revisão em si) fica por conta do Eduardo — não mexer sem pedido explícito.** Falta: confirmar nome do operador, capturas de tela finais (6 telas já roteirizadas e verificadas visualmente), conferir/aprovar o serviço `editais-legal` no Render, enviar `.aab` + ficha no Play Console.
